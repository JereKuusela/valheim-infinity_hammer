...
public class HammerCommand {
  ...
  private static GameObject SetItem(Terminal terminal, string name) {
    ...
    if (!Hammer.Set(Player.m_localPlayer, prefab, null)) throw new InvalidOperationException("Invalid prefab.");
    return prefab;
  }
  private static GameObject SetHoveredItem() {
    var view = GetHovered();
    var name = Utils.GetPrefabName(view.gameObject);
    if (!Hammer.Set(Player.m_localPlayer, view.gameObject, view.GetZDO())) throw new InvalidOperationException("Invalid prefab.");
