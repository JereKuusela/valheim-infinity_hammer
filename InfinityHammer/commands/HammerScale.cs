...
public class HammerScaleCommand {
  private static void Command(string direction, Action<float> action) {
    ...
    Helper.Command($"hammer_scale_{direction}", $"[percentage] - Scales the {direction} axis (if the object supports it).", (args) => {
  ...
  public HammerScaleCommand() {
    ...
    Helper.Command("hammer_scale", "[amount] - Scales the selection (if the object supports it).", (args) => {
      if (args.Length < 2)
        Scaling.SetScale(1f);
      else {
        var amount = Parse.Direction(args.Args, 2) * Parse.TryFloat(args[1], 0.05f);
