...
public class HammerSaveCommand : TextReceiver
{
  ...
  private static string GetExtraInfo(GameObject obj, DataEntry data)
  {
    ...
    if (data.TryGetString(pars, ZDOVars.s_tag, out var tag))
      info = tag;
    if (obj.GetComponent<ItemStand>() && HammerHelper.TryGetName(data, ZDOVars.s_item, out var item))
    {
    ...
    if (obj.TryGetComponent<ArmorStand>(out var armorStand))
    {
      ...
      var slots = armorStand.m_slots.Select(slot => $"{HammerHelper.ConvertHashToName(slot.m_visualHash)}:{slot.m_visualVariant}");
